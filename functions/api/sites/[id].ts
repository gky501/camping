...
export const onRequestPatch: PagesFunction<Env> = async ({ env, request, params }) => {
  ...
  try { return await updateSite(env.DB, String(params.id || ''), await request.json<JsonObject>()); } catch (cause) { return error(cause instanceof Error ? cause.message : 'Unexpected error', 500); }
};

...
function camperStats(camper: CamperProfile, stays: Stay[], sites: Campsite[]) {
  ...
  return {
    ...
    upcomingTrips: trips.filter((stay) => tripStatus(stay) !== 'completed').length,
...
function specItems(camper: CamperProfile): Array<[string, string]> {
  const items: Array<[string, string | undefined]> = [
    ['Type', camperTypeLabel(camper.type)],
    ['Year', camper.year ? String(camper.year) : undefined],
    ['Make', camper.make],
    ['Model', camper.model],
    [camper.type === 'tent' ? 'Footprint length' : 'Length', camper.lengthFeet ? `${camper.lengthFeet} ft` : undefined],
    ['Sleeps', camper.sleeps ? String(camper.sleeps) : undefined],
    ['Tent style', camper.type === 'tent' ? camper.tentStyle : undefined],
    ['Slide-outs', camper.type !== 'tent' && camper.slideOuts !== undefined ? String(camper.slideOuts) : undefined],
    ['Dry weight', camper.type !== 'tent' && camper.dryWeightLbs ? `${camper.dryWeightLbs.toLocaleString()} lbs` : undefined],
    ['GVWR', camper.type !== 'tent' && camper.gvwrLbs ? `${camper.gvwrLbs.toLocaleString()} lbs` : undefined],
  ];
  return items.filter((item): item is [string, string] => Boolean(item[1]));
}

function CamperDetailModal({ camper, stays, sites, onClose, onEdit }: {
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
      <div className="modal-card camper-detail-modal" role="dialog" aria-modal="true" aria-labelledby="camper-detail-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <section className="form-section camper-profile-hero">
          <div><span className="camper-type-badge"><TentTree size={16} /> {camperTypeLabel(camper.type)}</span><h3>{camperSubtitle(camper)}</h3><p>{camper.notes || 'No camper notes added yet.'}</p></div>
          <div className="camper-stat-strip"><span><strong>{stats.completedTrips}</strong> trips</span><span><strong>{stats.nights}</strong> nights</span><span><strong>{stats.parks}</strong> parks</span>{stats.upcomingTrips > 0 && <span><strong>{stats.upcomingTrips}</strong> upcoming</span>}</div>
        </section>
...
export function CampersPanel({ campers, stays, sites, onAdd, onEdit, onDelete }: {
  campers: CamperProfile[];
  stays: Stay[];
  sites: Campsite[];
  onAdd: () => void;
  onEdit: (camper: CamperProfile) => void;
  onDelete: (camper: CamperProfile) => void;
}) {
  ...
  return (
    <section className="content-page">
      <div className="page-heading"><div><p className="eyebrow">Your camping setups</p><h2>Campers</h2><p>Create a profile for every camper you have used—including your Vintage, current RV, and tent camping—and tag each diary entry with the setup used.</p></div><button type="button" className="primary-button" onClick={onAdd}><Plus size={18} /> Add camper</button></div>
      <div className="summary-grid camper-summary-grid"><div className="summary-card"><TentTree /><div><strong>{campers.length}</strong><span>Camper profiles</span></div></div><div className="summary-card"><Moon /><div><strong>{totalNights}</strong><span>Tagged completed nights</span></div></div><div className="summary-card"><BookOpen /><div><strong>{stays.filter((stay) => stay.camperId).length}</strong><span>Tagged diary entries</span></div></div></div>
      ...
      <div className="camper-card-grid">{filtered.map((camper) => { const stats = camperStats(camper, stays, sites); return <article className="camper-card" key={camper.id}><div className="camper-card-heading"><div><span className="camper-type-badge"><TentTree size={14} /> {camperTypeLabel(camper.type)}</span><h3>{camper.name}</h3><p>{camperSubtitle(camper)}</p></div></div><div className="camper-card-stats"><span><strong>{stats.completedTrips}</strong> trips</span><span><strong>{stats.nights}</strong> nights</span><span><strong>{stats.parks}</strong> parks</span></div>{stats.upcomingTrips > 0 && <span className="trip-status-pill upcoming">{stats.upcomingTrips} upcoming</span>}<p className="camper-card-note">{camper.notes || 'No profile notes yet.'}</p><div className="button-row camper-card-actions"><button type="button" className="secondary-button" onClick={() => setSelected(camper)}><Eye size={16} /> View</button><button type="button" className="secondary-button" onClick={() => onEdit(camper)}><Pencil size={16} /> Edit</button><button type="button" className="danger-button" disabled={camper.id === 'camper-tent'} title={camper.id === 'camper-tent' ? 'The Tent Camping profile is kept available' : 'Delete camper profile'} onClick={() => onDelete(camper)}><Trash2 size={16} /></button></div></article>; })}</div>
      ...
      {selected && <CamperDetailModal camper={selected} stays={stays} sites={sites} onClose={() => setSelected(undefined)} onEdit={() => { setSelected(undefined); onEdit(selected); }} />}
    </section>
  );
}
